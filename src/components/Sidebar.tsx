...
const Sidebar: React.FC = () => {
  ...
  const { cart, clearCart, total, itemAmount, checkoutCart } = useContext(CartContext);
  const [buttonText, setButtonText] = useState<string>("Passer la commande");
  ...
  return (
    <div className={`${isOpen ? 'right-0' : '-right-full'} w-full bg-white fixed top-0 h-full shadow-2xl md:w-[35vw] xl:max-w-[30vw] transition-all duration-300 z-20 px-4 lg:px-[35px]`}>
      <div className='flex-items-center justify-between py-6 border-b'>
        <div className='uppercase text-sm font-semibold'>Panier ({itemAmount})</div>
        <div
          onClick={handleClose}
